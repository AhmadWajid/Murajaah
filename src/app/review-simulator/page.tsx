...
import { SCHEDULERS, type AlgorithmType } from '@/lib/reviewAlgorithms';
...
export default function ReviewSimulator() {
  ...
  return <main className="mx-auto max-w-[1600px] space-y-7 p-6 text-foreground">
    ...
    <p className="text-sm">Easy: fluent unaided recall · Medium: effortful unaided recall · Hard: needed help or forgot. One day means the next calendar date. Same-day successes keep memory and schedule; Hard still updates them. Beginner exits after five successful reviews on distinct days; Hard restarts progress.</p>
    ...
    <div className={`grid gap-6 ${compare ? 'xl:grid-cols-3' : ''}`}>
      {visible.map(key => {
        ...
        return <section key={key} className="min-w-0 space-y-4 border-t border-border pt-4">
          <h2 className="text-xl font-semibold">{SCHEDULERS[key].name}</h2><p>Interval {state.interval} days · due {state.nextReview}<br />Beginner {state.isBeginner ? 'ON' : 'OFF'} · {state.reviewCount} reviews</p>
          ...
          <div className="overflow-x-auto"><table className="w-full whitespace-nowrap text-left text-xs"><caption className="mb-2 text-left">Review transitions (days; memory metrics when applicable)</caption><thead><tr>{['# / date / rating', 'Interval / next due / days away', ...SCHEDULERS[key].metrics, 'Count / beginner'].map(label => <th className="p-2" key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((row, i) => <tr key={i} className="border-t border-border"><td className="p-2">{i + 1} · {row.event.date}<br />{row.event.rating}</td><td className="p-2">{transition(row.before.interval, row.after.interval)}<br />{row.after.nextReview}<br />{DateTime.fromISO(row.after.nextReview, { zone }).diff(DateTime.fromISO(row.event.date, { zone }), 'days').days} days away</td>{SCHEDULERS[key].metrics.map(metric => <td className="p-2" key={metric}>{transition(row.before[metric], row.after[metric])}</td>)}<td className="p-2">{row.before.reviewCount} → {row.after.reviewCount}<br />{row.before.isBeginner ? 'ON' : 'OFF'} → {row.after.isBeginner ? 'ON' : 'OFF'}</td></tr>)}</tbody></table></div>
