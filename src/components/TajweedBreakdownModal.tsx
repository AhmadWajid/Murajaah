  ...
  useEffect(() => {
    ...
    getTajweedWords(surah, ayah)
      .then((words: TajweedWord[]) => {
        ...
        const occurrences: { ruleClass: string; text: string; touchesEnd: boolean }[] = [];
        words.forEach((word) => {
        ...
        occurrences.forEach(({ ruleClass, text }) => {
          const existing = byClass.get(ruleClass);
          if (existing) {
            existing.count++;
            ...
            byClass.set(ruleClass, { ruleClass, count: 1, samples: [text] });
          }
        });
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {rules.map((rule) => {
                ...
                const colorClass = TAJWEED_COLORS[rule.ruleClass] || 'text-gray-600';
