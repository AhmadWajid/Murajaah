  ...
  const arabicTextContainer = (
    <div className="text-right mb-4 sm:mb-6 px-1 sm:px-0 overflow-visible">
      {shouldShowHidden ? (() => {
        ...
        if (!arabicText) {
          return (
            <div className="flex flex-col items-center justify-center gap-3 py-8">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-[var(--radius-sm)] bg-warning/8 border border-warning/15 text-warning text-xs font-semibold font-sans">
              ...
              <button
                onClick={(e) => { e.stopPropagation(); onRevealMistake?.(surahNumber, ayahNumber); }}
                className="text-xs font-semibold text-accent hover:text-accent transition-colors font-sans"
              >
                Reveal verse →
              </button>
            </div>
        ...
        // Render the verse with hideWords + red dashed boxes to match the
        // hide-words visual but with warning/error semantic color.
        let textToRender = arabicText;
        if (isFirstAyah && surahNumber !== 1 && surahNumber !== 9) {
          if (surahNumber === 36) {
            const yaseenIndex = arabicText.indexOf('يسٓ');
            if (yaseenIndex !== -1) textToRender = arabicText.substring(yaseenIndex);
          } else {
            textToRender = removeBismillah(arabicText) || arabicText;
          }
        }
        return (
          <div className="relative">
            <TajweedAyahText
              ayahText={textToRender}
            ...
            <div className="flex items-center justify-center gap-2 mt-3">
              <span className="flex items-center gap-1.5 text-[10px] font-semibold text-warning font-sans uppercase tracking-wider">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                </svg>
                Mistake — test your memory
              </span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-[var(--radius-xs)] bg-accent/10 hover:bg-accent/15 border border-accent/20 text-accent text-[11px] font-semibold transition-colors font-sans"
              >
                Reveal
              </button>
            </div>
